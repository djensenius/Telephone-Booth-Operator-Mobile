...
//
//  Encodes recorded or imported operator audio into the FLAC container the
//  booth pipeline expects and computes its upload metadata.
//
...
public struct OperatorAudioFile: Sendable, Equatable {
    public let url: URL
...
public enum OperatorAudioError: LocalizedError {
    case emptyAudio
...
public enum OperatorAudioEncoder {
    ...
    public static func encodeToFLAC(source: URL) async throws -> OperatorAudioFile {
        try await Task.detached(priority: .userInitiated) {
            try encodeToFLACSync(source: source)
        }.value
    }

    private static func encodeToFLACSync(source: URL) throws -> OperatorAudioFile {
        ...
        do {
            ...
            throw OperatorAudioError.unreadableSource
        }

        let format = input.processingFormat
        guard input.length > 0 else { throw OperatorAudioError.emptyAudio }

        let durationMs = Int((Double(input.length) / format.sampleRate) * 1000.0)
        guard durationMs <= maxDurationMs else {
            throw OperatorAudioError.tooLong(maxMinutes: maxDurationMs / 60_000)
        }

        let outURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("operator-audio-\(UUID().uuidString).flac")
        ...
        } catch let error as OperatorAudioError {
            throw error
        } catch {
            throw OperatorAudioError.encodingFailed(error.localizedDescription)
        }
        ...
        guard !data.isEmpty else { throw OperatorAudioError.emptyAudio }

        let digest = SHA256.hash(data: data)
        let sha256 = digest.map { String(format: "%02x", $0) }.joined()

        return OperatorAudioFile(
            url: outURL,
            data: data,
            sha256: sha256,
    ...
    private static func writeFLAC(
        ...
        guard let buffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: frameCapacity) else {
            throw OperatorAudioError.encodingFailed("buffer allocation failed")
        }
